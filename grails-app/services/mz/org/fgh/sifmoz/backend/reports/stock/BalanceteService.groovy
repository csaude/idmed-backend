...
@Transactional
@Service(BalanceteReport)
abstract class BalanceteService implements IBalanceteService {
    @Autowired
    ...
    List<BalanceteReport> getBalanceteReportById(String reportId) {
        return BalanceteReport.findAllByReportId(reportId)
    }
    ...
    void processReport(ReportSearchParams searchParams, ReportProcessMonitor processMonitor) {
        ...
        def result = packService.getBalanceteReport(clinicalServiceParams, clinicParams, searchParams.getStartDate(), searchParams.getEndDate())
