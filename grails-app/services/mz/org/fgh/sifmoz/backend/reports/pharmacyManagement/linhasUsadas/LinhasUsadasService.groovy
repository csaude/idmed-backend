...
import mz.org.fgh.sifmoz.backend.reports.patients.ExpectedPatientReport
...
@Transactional
@Service(LinhasUsadasReport)
abstract class LinhasUsadasService implements ILinhasUsadasService{
    @Autowired
    ...
    List<LinhasUsadasReport> getLinhasUsadasReportById(String reportId) {
        return LinhasUsadasReport.findAllByReportId(reportId)
    }
    ...
    void processReport(ReportSearchParams searchParams, ReportProcessMonitor processMonitor) {
            ...
            def result = packService.getLinhasUsadas(clinicalServiceParams, clinicParams, searchParams.getStartDate(), searchParams.getEndDate())
            ...
            if (result.size() == 0) {
                ...
                if (Utilities.listHasElements(result)) {
                    ...
                    for (linhaUsadaReport in result) {
